...
fun Farmer.toEntity(): FarmerEntity {
    return FarmerEntity(
        ...
        phone = contactInfo?.primaryPhone ?: phone,
        ...
        landSize = farmInfo?.totalLandSize ?: 0.0,
        landUnit = farmInfo?.landUnit?.name ?: "ACRES",
        farmingType = farmInfo?.farmingType?.name ?: "CROP_FARMING",
        experienceYears = farmInfo?.farmingExperienceYears ?: 0,
        profileImageUrl = profileImageUrl,
        status = try { status?.name ?: "ACTIVE" } catch (_: Exception) { "ACTIVE" },
        lastSyncTime = lastSyncTime,
        createdAt = createdAt,
        updatedAt = updatedAt,
        userRole = try { userRole?.name ?: "FARMER" } catch (_: Exception) { "FARMER" }
    )
}
